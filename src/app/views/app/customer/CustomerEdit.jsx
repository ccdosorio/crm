...
class CustomerEdit extends Component {
  ...
  getData = async () => {
    const res = await Http.get(`customer/${this.props.match.params.id}`);
    await this.setState({
      form: res[0],
      hasRecords: true,
      onPropValidate: true,
  ...
  handleSubmit = async (event) => {
    ...
    const res = await Http.post("customer-put", customer);
    await this.setState({ show: true, msg: res });
    await this.props.history.push(
      `/customer/${this.props.match.params.id}/edit`
    );
  };
  ...
  render() {
    ...
    let title = "";
    let msg;
    user.rol === -1 ? (title = "Cliente") : (title = "Editar Cliente");
    ...
    return (
      <React.Fragment>
        <Breadcrumb
          routeSegments={[
            { name: "Regresar", path: "/customer/list" },
            { name: title },
          ]}
        />
        {this.state.show && (
          ...
          <div className="row">
            ...
            <div className="col-2">
              <MenuOptions
                ...
                client={this.state.form.idcliente}
                type={this.state.form.idcliente_tipo}
              />
            </div>
            {user.rol !== -1 && (
