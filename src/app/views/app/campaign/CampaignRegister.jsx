...
class CampaignRegister extends Component {
  ...
  uploadSingleFile = (index) => {
    console.log(this.state.files_content);
    ...
    let campaign = this.props.match.params.idcampaign;
    const file_event = this.state.files[index].file;
    Storage.put(
      `nd/campanias/${campaign}/${binder}/${file_event.name}`,
      file_event,
      {
      ...
      .then((res) => {
        if (res) {
          ...
          let campaign = {
            idcampaign: this.props.match.params.idcampaign,
            name: name_file[0],
            path: res.key,
            type: file_event.type,
          };
          this.saveDocumentDB(campaign);
        }
      })
      .catch((err) => {
        console.log(err);
  ...
  uploadAllFile = () => {
    ...
    for (let index = 0; index < this.state.files.length; index++) {
      ...
      campaign = this.props.match.params.idcampaign;
      const file_event = this.state.files[index].file;
      Storage.put(
        `nd/campanias/${campaign}/${binder}/${file_event.name}`,
        file_event,
  ...
  handleSingleCancel = (index) => {
    ...
    let campaign = this.props.match.params.idcampaign;
    const file_event = this.state.files[index].file;
    Storage.remove(`nd/campanias/${campaign}/${binder}/${file_event.name}`)
      .then((res) => {
        if (res) {
          this.setState({
            files: [...allFiles],
          ...
          this.deleteDocumentDB(campaign);
        }
      })
      .catch((err) => {
        console.log(err);
  ...
  saveDocumentDB = async (campaign) => {
    await Http.post("document-campaign", campaign);
  };

  deleteDocumentDB = async (campaign) => {
    await Http.post("document-campaign/delete", campaign);
  };
